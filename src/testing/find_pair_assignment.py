...
def find_pair_assign(ds, result_path, eval_path, algo, alpha, beta):
    pair1, pair2 = ds.load()

    with open(join(result_path, f"{ds.name}_resilient_{ds.k}_{algo}({alpha}_{beta}).pickle"), 'rb') as input_file:
    ...
    with open(join(eval_path, f"{ds.name}_resilient_{ds.k}_{algo}({alpha}_{beta})_eval.pickle"), 'wb') as output_file:
